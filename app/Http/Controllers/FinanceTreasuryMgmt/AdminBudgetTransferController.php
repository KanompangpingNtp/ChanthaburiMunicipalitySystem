...
class AdminBudgetTransferController extends Controller
{
    public function BudgetTransferAdmin()
    {
        $perfResultsType = PerfResultsType::all();
        $perfResultsTypeID = $perfResultsType->firstWhere('type_name', 'โอนเงินงบประมาณรายจ่าย')->id;
        $PerfSingleTopic = PerfSingleTopic::where('perf_results_type_id', $perfResultsTypeID)->get();

        return view('admin.post.finance_treasury_mgmt.budget_transfer.page', compact('PerfSingleTopic', 'perfResultsType'));
    }

    public function BudgetTransferCreate(Request $request)
    {
        $request->validate([
    ...
    public function BudgetTransferUpdate(Request $request, $id)
    {
        $request->validate([
            'detail_name' => 'required|string',
        ]);
    ...
    public function BudgetTransferDelete($id)
    {
    ...
    public function BudgetTransferShowDetails($id)
    {
        $PerfSingleTopic = PerfSingleTopic::with('files')->findOrFail($id);

        return view('admin.post.finance_treasury_mgmt.budget_transfer.show_details', compact('PerfSingleTopic',));
    }

    public function BudgetTransferCreateFiles(Request $request, $DetailsId)
    {
        $request->validate([
    ...
    public function BudgetTransferDetailsDelete($fileId)
    {
