...
class AdminPlanProgressController extends Controller
{
    public function PlanProgressAdmin()
    {
        $perfResultsType = PerfResultsType::all();
        $perfResultsTypeID = $perfResultsType->firstWhere('type_name', 'แผนและความก้าวหน้าในการดำเนินงานและการใช้จ่ายงบประมาณประจำปี')->id;
        $PerfSingleTopic = PerfSingleTopic::where('perf_results_type_id', $perfResultsTypeID)->get();

        return view('admin.post.operation.planprogress.page', compact('PerfSingleTopic', 'perfResultsType'));
    }

    public function PlanProgressCreate(Request $request)
    {
        $request->validate([
    ...
    public function PlanProgressUpdate(Request $request, $id)
    {
        $request->validate([
            'detail_name' => 'required|string',
        ]);
    ...
    public function PlanProgressDelete($id)
    {
    ...
    public function PlanProgressShowDetails($id)
    {
        $PerfSingleTopic = PerfSingleTopic::with('files')->findOrFail($id);

        return view('admin.post.operation.planprogress.show_details', compact('PerfSingleTopic',));
    }

    public function PlanProgressCreateFiles(Request $request, $DetailsId)
    {
        $request->validate([
    ...
    public function PlanProgressDetailsDelete($fileId)
    {
