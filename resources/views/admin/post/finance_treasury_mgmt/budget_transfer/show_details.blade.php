...
<h2 class="text-center">สร้างรายละเอียด <br><span class="text-primary">{{$PerfSingleTopic->detail_name}}</span></h2><br>
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('WorkforcePlanCreateFiles', $PerfSingleTopic->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
...
<table class="table table-bordered text-center">
    ...
    <tbody>
        @foreach($PerfSingleTopic->files as $index => $file)
        <tr>
            <td>{{ $index + 1 }}</td>
            ...
            <td class="text-center align-middle">
                <div class="d-flex justify-content-center gap-2">
                    ...
                    <form action="{{ route('WorkforcePlanDetailsDelete', $file->id) }}" method="POST" onsubmit="return confirm('คุณต้องการลบไฟล์นี้หรือไม่?');">
                        @csrf
                        @method('DELETE')
